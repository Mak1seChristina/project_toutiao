...
export default {
  name: 'User',
  methods: {
    ...mapActions('userAbout', ['initUserInfo']),
    ...mapMutations('userAbout', ['cleanState']),
    async logout() {
      // 确认登出的提示框
      const confirmResult = await this.$dialog
        ...
        .catch(err => {
          // 把 err return 给 confirmResult
          return err
        })

      // 判断点击的是 确认 还是 取消
      // 如果是 取消 直接 return 不执行后续操作
      if (confirmResult === 'cancel') return

      // 点击了 确认
      // 清空 vuex 中的 state 并清空本地存储的数据
      this.cleanState()
      // 跳转到登录页面
